...
if len(file) == 0:
    ...
    cols = df[['Pop. Density (per sq. mi.)', 'Coastline (coast/area ratio)', 'Net migration',
    ...
    purpose = st.sidebar.selectbox("Choose an Option",
                                   ["Information about the Dataset", "Countries' Tables", "Visualize the Data",
    ...
    if purpose == "Information about the Dataset":
        ...
        st.write(
            "This Dataset contains demoghraphic information about the world's countries. Some of the dataset's content include population, area size, population density, region, GDP, infant mortality, literacy rates, death rates, ")
        ...
        st.write(
            '**Region:** Shows the region in which the countries belongs to. For example, Latin America, Western Europe, Northern Africa, etc.')
        ...
        geo = df[['Country', 'Region', 'Area (sq. mi.)', 'Coastline (coast/area ratio)', 'Arable (%)', 'Crops (%)',
                  'Other (%)']]
        demo = df[['Country', 'Population', 'Pop. Density (per sq. mi.)', 'Net migration',
                   'Infant mortality (per 1000 births)', 'Birthrate', 'Deathrate']]
        socioeco = df[['Country', 'GDP ($ per capita)', 'Literacy (%)', 'Phones (per 1000)', 'Agriculture', 'Industry',
                       'Service']]
        ...
        x = df.loc[:,
            ["Region", "GDP ($ per capita)", "Infant mortality (per 1000 births)", "Birthrate", "Phones (per 1000)",
             "Literacy (%)"]]
        ...
        for feature_name, slider in sliders.items():
            ...
            filter = (
                    filter & (df['GDP ($ per capita)'] >= sliders.get("GDP")) & (
                        df['GDP ($ per capita)'] <= sliders.get("GDP1"))
        ...
        fig = px.bar(mena, x="country", y="visa_free_score", color="country", animation_frame="year",
